...
4. Docker build + gerber merge into `boards/microcore-m/` (revision letter from `revision.txt`)
5. Commit/push fab outputs when on `main`

A pull-request branch may *run* Create Board; **fab files are only pushed when the workflow runs on `main`**. After merging rev **m**, the next green run on `main` should create `boards/microcore-m/`. Manual path: GitHub → Actions → **Create Board** → Run workflow (on `main`).
