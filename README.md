...
Spec locked in [docs/SPEC.md](docs/SPEC.md). Hardware on this branch: **rev l** (F.Cu+B.Cu GND pours refilled; Core EFI silkscreen). Last Create Board packages on main include `boards/microcore-j/`. `boards/microcore-l/` lands when Create Board succeeds on `main` after merge. See [docs/HARDWARE_STATUS_rev_l.md](docs/HARDWARE_STATUS_rev_l.md). **Not production-ready.**
...
KiCad board basename: `microcore` (`BOARD_PREFIX=micro`, `BOARD_SUFFIX=core`, rev `l`).
...
- Fab outputs land under `boards/microcore-j/` (last on main). After merge, Create Board writes `boards/microcore-l/`
