...
| `silk_over_copper` | 53 | Frame silk on Q/R/D, including the D9–D14 back silk noted on rev p. | later |
| `silk_over_copper` | 28 | Hellen M2. | later |
| `silk_overlap` | 19 | Frame reference silk (injector FETs, ballast resistors, LS FETs). | later |
...
| `holes_co_located` | 1 | Frame. Two identical `/INJ1` vias. | later |
...
| Schematic `footprint_symbol_mismatch` | 32 | Frame library-id string. Embedded name is `DPAK` / `R0603` / `DO214AC`; the symbol says `microcore-fp:...`. Same land. D1–D14, Q1–Q7, R1–R11. | later |
...
**Duplicate via (not changed)**

Two board-level vias, both `/INJ1`, both `(174.2, 90.35)`, size 0.8, drill 0.4. UUIDs `7e5055f5-3e87-43d5-ba01-377e73eca3d2` and `bb928a49-411f-561d-bb74-63aa78d8f3f6`. Same net, so this is one extra drill mark, not a short. Left for a later cleanup.

## What still blocks a fab order

Nothing beyond Hellen noise and a human copper review.

`boards/microcore-q/` is already on `main`. This pass did not add a short, a clearance error, or a frame unconnected item. The remaining DRC errors are module keepout hits, the four pad-G shorts, the M3 E4 clearance/mask pair, the M3 edge hit, and M2 pad S1. The duplicate `/INJ1` via and the silk/library-id warnings are not electrical blocks. A person should still look at the rev q gate-driver copper and at this stitch before ordering.
