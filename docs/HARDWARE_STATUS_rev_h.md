...
**Create Board artifacts:** last green `boards/microcore-i/` (CI run 35559694730 success).
...
| 5 | Q1–Q7 + gate Rs on JLC BOM with LCSC | **Done on `boards/microcore-i/` JLC BOM:** Q1–Q7=`C155647`, R1–R7=`C22783`. |
| 6 | Create Board after PCB change; bump revision | **BOARD_REVISION=i**; Create Board **green** → `boards/microcore-i/`. |

## Inventory

| Area | Present |
|---|---|
| Last green board package | `boards/microcore-i/` gerber/BOM/CPL/ibom/png/pdf |
...
Flyback/TVS layout vs SPEC defer is with Quintin via Boss — **do not start either path** until Boss confirms.
...
3. ~~Confirm Create Board `boards/microcore-i/`~~ — **done** (green; Q/R on JLC BOM).
...
- README → microcore-i (last green); this status doc.  
