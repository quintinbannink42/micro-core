...
## Power stage
7× DPAK-class N-FET, all **AOD4184A** (TO-252, LCSC **C99124**) on the existing `microcore-fp:DPAK` land. Quintin chose this part, and TVS on the low-side channels, for budget and reliability.
...
3× low-side (Q5 fuel pump, Q6 idle / VVT, Q7 boost / spare) with gate R5–R7 at 12 ohm, plus the same cheap injector clamps per channel: US1M flyback (anode on the LS net, cathode on +12 V) and SMAJ33A (cathode on the LS net, anode on GND). No series ballast on Q5–Q7. Pins 8/9/10 are those drains.

AOD4184A is a discrete MOSFET, not a protected smart FET. RDS(on) max is 9.5 mohm at Vgs = 4.5 V (not specified at 3.3 V). Vth max is 2.6 V, so a 3.3 V MCU gate turns it on but may not hit the 4.5 V RDS(on) spec. No gate driver on this rev. See `docs/HARDWARE_STATUS_rev_p.md`.
