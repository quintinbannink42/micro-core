# Micro Core — product spec (rev P, 2026-09-24)
...
## Power stage
7× DPAK-class N-FET, all **NCE4080K** (TO-252, LCSC **C191380**) on the existing `microcore-fp:DPAK` land. Budget and reliability first: no FET or BOM upgrade, and the land is not redrawn. Low-side channels get the same cheap US1M + SMAJ33A clamps already on the board.
...
NCE4080K is a discrete MOSFET, not a protected smart FET. RDS(on) is < 7 mohm at Vgs = 10 V and ID = 20 A. There is no RDS(on) row at 4.5 V or 2.5 V. Vth is 1.2 / 1.8 / 2.5 V (min / typ / max), so a 3.3 V MCU gate turns it on but does not hit the 10 V RDS(on) spec. No gate driver on this rev. See `docs/HARDWARE_STATUS_rev_p.md`.
...
`bom_replace_microcore-p.csv` keeps `U105` `STM32F407VGT6` / `C12345` and keeps Q1–Q7 as `NCE4080K` / `C191380`. D9–D11 are US1M / C112545 and D12–D14 are SMAJ33A / C143131. C15815 and C2252 are not this MCU (op-amp and 22.1184 MHz crystal).
